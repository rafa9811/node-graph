...
/* Devuelve la lista de ids de nodos contenidos en el grafo usando nueva memoria para el puntero que se devuelve (quien llame a esta función se tiene que encargar de liberar dicho puntero al terminar). 
Si hay algún error se devuelve NULL */
int* graph_getNodeIds(const Graph * g);
/* Devuelve el número de conexion es que se almacenan en un grafo, o -1 si se produce algún error */
int graph_getNedges(const Graph * g);
/* Se añade un nodo al grafo reservando memoria nueva para dicho nodo, actualizando los atributos internos del grafo que sean necesarios. 
 * Se devuelve el grafo actualizado si todo ha ido bien, o NULL en otro caso */
...
/* Imprime en pf los datos de un grafo, devuelve el número de caracteres que se han escrito con éxito (mirar documentación de fprintf). 
 * El formato a seguir es: imprimir una primera línea con el número de nodos y conexiones, después, se imprime una línea por nodo, 
 * incluyendo la información del nodo y las conexiones con el resto de nodos como 1 o 0. La salida para el grafo pintado enel ejercicio 2.3 de la parte 1 es: N=3, E=3: [1, a]->0 1 1 [2, b]->0 0 1 [3, c]->0 0 0  */
